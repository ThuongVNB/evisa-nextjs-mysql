...
import Xref_visa_country from '@/models/Xref_visa_country';

export const POST = async (request) => {
    const xrefData = await request.json();
    try {
        const result = await Xref_visa_country.create(xrefData);
        return new NextResponse('The country has been allowed', { status: 201 });
    } catch (error) {
...
export const PUT = async (request) => {
    ...
    try {
        const result = await Xref_visa_country.update(
            { post_id, category_id },
            {
